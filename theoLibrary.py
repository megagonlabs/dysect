...
def loadParameters(propertiesFileName, knowledgeBaseID, lastIteration):
    if knowledgeBaseID != 'none':
        propertiesJsonFileName = "data/kbs/"+ str(knowledgeBaseID) + "/" + str(propertiesFileName) 
...
def kbStorageInitialization(knowledgeBaseID):
    if not os.path.exists("data/kbs/"+ str(knowledgeBaseID) + "/prompts/"):
    ...
    print('New empty KB succefully created.')
...
def knowledgeIntegrator(knowledgeBaseID,iteration, rootConcept, frequencyThreshold, promotionCriteria):
    pass #TODO: go through all the concepts and relations to be learned and inspect their specializations
    print('#TODO: go through all the concepts and relations to be learned and inspect their specializations')
    ...
    if conceptsToInspect != None:
        for conceptToInspect in conceptsToInspect:
            pass
            #TODO:splitAndMergeConceptsAndRelations(kbElement,knowledgeBaseID,iteration,currentDate, maxNumberOfValues)
...
def probevLLM(promptForVllm):
    prompts = [vLLMlib.tokenizer.apply_chat_template([{"role": "user", "content": promptForVllm}], tokenize=False),
    vLLMlib.tokenizer.apply_chat_template([{"role": "user", "content": promptForVllm}], tokenize=False)]
    #Just test the vLLM
    #vLLM.run('test_inputs_v0.jsonl', 'test_outputs_v1.jsonl','meta-llama/Llama-2-7b-chat-hf',0.,1.0,128,None,'model_input')
    responses = vLLMlib.llm.generate(prompts, sampling_params=vLLMlib.sampling_params)
    ...
    return listOfGeneratedOutputs, vLLMlib.llm.llm_engine.model_config.model
...
def addValueInverseAndTypes(relationSubject,relationPredicate,relationObject,knowledgeBaseID,currentIteration,sourceOfInformation,currentDate, confidence, promptTemplate, promptInstance):
    ...
    addValue(relationSubject+'||'+relationPredicate+'||'+relationObject+'||iteration||'+str(currentIteration)+'||source||'+str(sourceOfInformation)+'||date||'+str(currentDate),'confidence',str(confidence), knowledgeBaseID)
    #add +1 in the count of how many times this piece of knowledge was extracted
    ...
    if len(tempRelationPredicate) == 1: #it means that the relation is "generalizations" or "specializations"
        ...
        #add the generalizations for the relation/predicate
        ...
        #add the specializations for the relation/predicate as an instance of relation
